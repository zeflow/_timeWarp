...
def build_analyze_panel_js() -> str:
    ...
    <section style="margin-top:16px;">
      <div style="display:flex;gap:12px;margin-bottom:12px;">
        ...
        <button id="tab-analyze" class="secondary" type="button">Analyze</button>
      </div>
      <div id="panel-logs">
    ...
    <script>
      ...
      function activate(tab) {
        ...
        panelAnalyze.classList.toggle('hidden', logsActive);
        logsTab.classList.toggle('secondary', !logsActive);
        analyzeTab.classList.toggle('secondary', logsActive);
      }
        ...
        for (const st of summary.sourcetypes) {
          ...
          datasets.push({
            ...
            data: [
              ...
              {x: end, y: st.sourcetype, endCap: true},
            ],
      ...
      function renderControls(summary, selected) {
        ...
        for (const st of summary.sourcetypes) {
          ...
          wrap.querySelector('input').addEventListener('change', (e) => {
            if (e.target.checked) selected.add(st.sourcetype); else selected.delete(st.sourcetype);
      ...
      function loadSummary() {
        fetch('/scan/summary').then(r => r.json()).then(summary => {
          if (summary.error || !summary.sourcetypes) {
            document.getElementById('live-output').textContent = summary.error || 'No summary yet. Run a scan.';
            return;
          }
          const selected = new Set(summary.sourcetypes.slice(0,8).map(s => s.sourcetype));
          renderControls(summary, selected);
          renderTable(summary);
          renderChart(summary, selected);
          activate('analyze');
        }).catch(err => {
          document.getElementById('live-output').textContent = 'Failed to load summary: '+err;
        });
      }
...
@app.route("/analyze")
def analyze():
    # Keep page for direct access, but uses live summary from scan progress
    extra = build_analyze_panel_js()
    return render_page("Analyze", fields=None, action=None, extra_html=extra, task_slug="scan")
